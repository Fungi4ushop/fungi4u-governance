...
- **✅✅ AND THE PREMISE OF THE "BUY THE SUNSYNK BRIDGE" RECOMMENDATION WAS WRONG — IT IS ALREADY BUILT, DEPLOYED AND LIVE. DO NOT SPEND THE R750–1,100.** `SOLAR.md` records the custom **`sunsynk-openapi-addon`** as *"tested end-to-end… it just needs deploying"*; **it has since been deployed.** **Verified 2026-08-18: all 15 sensors present under `sensor.sunsynk_inverter_cloud_*` and updating on a ~5-minute poll** — PV, load, grid import/export, battery SOC/power/current/temperature, and the daily kWh totals. ➡️ **The Waveshare bridge would buy RESILIENCE and RESOLUTION, not information** *(no internet dependency, sub-minute polling, no custom code)*. **`SOLAR.md`'s own verdict stands: "coarse 5-min cloud data is fine for the actual purpose."**
  - **⚠️ THE REAL WEAKNESS IS THE ONE THE BRIDGE WOULD FIX, AND IT HAS ALREADY BITTEN: this add-on is CLOUD-DEPENDENT.** **The recorder holds Sunsynk history only to 2026-08-09 00:13 — the night the fibre authentication failed** *(§network)*. **Live now, but the outage and the subnet rebuild took it out and nobody noticed.** **That is `MICROCLIMATE.md`'s own standing warning — *"the monitoring itself must be checked periodically; it has failed silently before"* — landing on the solar side this time.**
  - **⬜ SO THE FREE ACTION IS HISTORY, NOT HARDWARE.** **Live values answer nothing about a lever that only shows over weeks**, and there is currently **no usable Sunsynk history at all.** **Confirm these entities are actually being recorded** *(recorder `include`/`exclude`)*, **and that the add-on survives a restart.** ➡️ **Then re-ask the battery question in summer, off real data, for R0.**
