...
- **➡️ What remains is the drift this file already predicts: the zero moves when the unit is CARRIED.** It was recalibrated outdoors on 08-08 and returned to the shelf — and this file records the zero moving twice before, once specifically "on return to the shelf." **This would be the third instance of the same pattern: correct outdoors, wrong once it is back inside.**
- **🔴 WHICH HAS A SHARPER CONSEQUENCE THAN A BAD READING: if the zero does not survive the trip, then "calibrate outdoors and carry it back" CANNOT WORK for this device** — the act of returning it destroys the calibration being sought. **That would make the Inkbird unusable as a CO2 *reference* by this method**, whatever care is taken. It remains sound for **temperature and RH shelf gradients**, which use different elements and show no such drift. **Tomorrow's outdoor read tests exactly this:** ~50–100 outdoors means the zero moved again after 08-08, and the workflow is the problem rather than its execution.
- **A second, independent reason to distrust sub-500: a fruiting room with a respiring crop cannot sit at outdoor ambient (~425).** Both sensors agreeing near 490–505 fits the old common-mode ~350 low error far better than it fits a genuinely clean room — **especially now that 25% of the passive exhaust has been closed, which should push CO2 up, not down.**
...
- **🌡️ GRAIN-SPAWN WARM INCUBATOR — built 2026-08-10. The cheapest available probe of the colonisation-temperature hypothesis, and the first record in these files of any part of the grain-spawn stage.**
  ...
  **🔴 THE DOMINANT RISK IS A STUCK-ON HEATER, NOT A COLD TUB — and the oversizing is what makes it fast.** Aquarium thermostats fail **closed**. At 150 W into 28 L the tub goes **24 °C → 30 °C in ~75 minutes** — where bacteria and Trichoderma take the jars — and an insulated, lidded tub has **no safe equilibrium above that.**
  ...
  - **✅ THE FIX IS TWO THERMOSTATS IN SERIES, NOT A SMALLER HEATER.** An **external thermostat controller** (Inkbird ITC-306/308 class, ~R400–600) upstream, **probe in the tub, set 24 °C**; the **heater's own dial set to ~28 °C** as a backstop. **Either one failing closed is survivable, because the other still cuts.** It also gives ±0.3 °C against the heater's ±2 °C — which matters, because at 12–19× oversizing **±2 °C is the swing actually delivered, so a "24 °C" setting can sit at 26 °C.**
