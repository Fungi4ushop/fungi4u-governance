...
- **➡️ CONSEQUENCE FOR THIS FILE: the phrase "since the fresh-air fan was removed" is FALSE wherever it appears** — it survives at line ~104 and line ~399 and should be read as *"since active CO2 **control** was abandoned"*, which is the true statement. ⬜ **Both need correcting, along with `HARDWARE_REFERENCE.md`:261 and `MICROCLIMATE.md`:120/138/140/392, which still say the position is empty.** A successor reading those would rebuild a fan that is already on the wall.
...
- **⛔⛔ THE TWO BULLETS THAT STOOD HERE ARE FALSIFIED — the 2026-08-11 outdoor read settles it. THE INKBIRD'S ZERO DOES SURVIVE BEING CARRIED.** *(They asserted "the zero moves when the unit is CARRIED", called a third recurrence of "correct outdoors, wrong once back inside", and concluded that **"calibrate outdoors and carry it back CANNOT WORK for this device"**, making it *"unusable as a CO2 reference by this method."*)*
  ...
  - **⚠️ QUALIFIED SAME DAY, 09:51 — THE RETURN LEG WAS THEN WATCHED, AND THE TRUTH IS IN BETWEEN.** The block above was written after observing only the **outbound** leg. On return the Inkbird overshot to 504, decayed to 446, and **stabilised oscillating in a 446–462 band** against a steady primary at 493–497. **Delta: −28 before the trip, −44 after — a ~16 ppm shift.**
    - **⛔ The STRONG form of the carry theory stays falsified:** ~16 ppm is an order of magnitude below the 190–330 collapse claimed, and the sensor **oscillates rather than collapsing**. It is not *"unusable as a CO2 reference."*
    - **⚠️ A WEAK form may be real:** a ~15–20 ppm perturbation after handling. **And it cannot be attributed** — with two sensors and no absolute anchor there is no way to tell whether the Inkbird settled low or the room genuinely is lower (the common-mode blindness of `DECISIONS.md` 2026-07-29).
    - **➡️ NET, AND THIS IS THE USABLE STATEMENT: the Inkbird is a travelling reference with a ~±30–40 ppm uncertainty** (≈20 from handling, plus the diurnal-ambient limit below). **Ample to detect the primary's 233 ppm shift; NOT enough to calibrate to the ppm.** Use it to answer *"has something moved?"*, never *"what exactly is the offset?"*
...
- **Circulation fan: ACDC Dynamics WF-150**, mains 230V induction, inline mixed-flow. **🔴 It is a 150 mm fan on the 110 mm duct (150→110 reducer each end), i.e. 3–5× the duct's design flow — `DECISIONS.md` 2026-07-17 warned against exactly this size-up. No airflow figure is published on the box, plate, manual or ACDC's site. Full working and the recommended sequence: `stock-control/docs/HARDWARE_REFERENCE.md` §Fan/duct mismatch.** **⚠️ RUNNING ON HIGH, and BOTH duct end caps (supply and return) are OPEN — corrected here 2026-07-28; this entry previously said "running on LOW, do not raise to high", which had been stale since 07-21 and told readers the actual state was forbidden.**
  ...
  - **The cost is crop quality, not power** — the WF-150 draws **45/55 W** *(✏️ corrected 2026-08-11; this line read "23/26 W", which are the specs of the **RAM 100 mm** unit `DECISIONS.md` 2026-07-17 **recommended but which was never installed** — the figure was carried across to the fan that actually went in)*, so the speed difference is trivial on the bill. What it spends is the core design bet: low-velocity, well-mixed, **no direct draft on the caps** (`MICROCLIMATE.md` KPI 5 — too much airflow dries caps and raises local VPD).
...
**✅ DECIDED 2026-08-12 (operator) — NO ROUTER BACKUP FILE WILL BE KEPT. RECOVERY IS BY RECONFIGURING FROM THE STORED SETTINGS TABLE ABOVE.** *"Will not rely on router backup. Will reconfigure based on stored info."* **This closes the open item rather than leaving it outstanding**, and it is the option this file was already arguing for: a TP-Link `.bin` is **tied to the firmware version that produced it** — after a firmware update the router will often refuse to restore it — so it is a shortcut, not a foundation. **The durable asset is the settings table above:** plain text, in git, readable by a successor, and it does not rot. Restoring from it by hand is the ~15-minute job that was actually done on 2026-08-10, so the method is proven, not theoretical.

- **➡️ THE CONSEQUENCE, AND IT IS THE WHOLE COST OF THE DECISION: the table above IS the backup, so it must stay correct.** Anything changed on the router — a channel, a reservation, the pool range, an SSID — is only recoverable if it is written here at the time. **A stale row in that table is now a silent recovery failure**, the same shape as the `pret208-4.2G` transposed digit that sat in `HARDWARE_REFERENCE.md`. Update the table in the same session as any router change.
- **⛔ Do NOT "helpfully" reintroduce a `.bin` export later.** It was considered and declined; two recovery paths that can disagree is worse than one that is known to be authoritative.

**➡️ So the complete recovery kit is: the table above, plus three secrets that must NOT be in git** (`SAFETY.md`) — the **`pret208-2.4G` WiFi password**, the **router admin password**, and the **Afrihost PPPoE username/password**. Put those in a **Bitwarden secure note**, which works on the free tier. **⬜ That secure note is now the ONLY outstanding piece of the recovery kit** — without it the table reconfigures the LAN but cannot re-authenticate the line.
