...
- **⛔ THE HEADLINE READING IS STILL A TRAP AND STAYS WITHDRAWN.** 08-17 (2,943 W) against 08-18 (1,035 W) looks like the **21 → 19 °C setpoint change saving ~1,900 W**. **It is not:** **08-18 was 12.1 °C outdoors against 5.7–8.8 on every other night**, so far less heating was needed regardless of setpoint. **Never read a heating load without the outdoor temperature beside it.**
- **✅✅ AND THE 08-17 OUTLIER IS EXPLAINED — OPERATOR, 2026-08-18: THE BOREHOLE PUMP WAS LEFT ON FROM SUNDAY. THIS TURNS THE METHOD'S BIGGEST WEAKNESS INTO ITS BEST VALIDATION.** ⚠️ *(I had written it up as "nothing explains it — a house load, most likely". Correct in kind, and the operator supplied the specific cause unprompted. **Ask before theorising; the answer was one question away.**)*
  - **📐 AND IT IS MEASURABLE FROM THE SAME SIGNAL, BECAUSE A FORGOTTEN PUMP RAISES THE FLOOR RATHER THAN ADDING CYCLES.** Hourly p10: **583 W at 08-16 12:00 → ~1,900–2,300 W from 13:00 Sunday straight through to ~07:00 Monday → 753 W by 09:00.** ➡️ **The borehole pump draws ~1,400 W continuous, and it ran ~19 hours ≈ 26 kWh ≈ R136 at R5.12/kWh, in one incident.**
  - **✅ INDEPENDENTLY CORROBORATED BY THE DAILY ENERGY COUNTERS: grid import on 08-16 (41.8 kWh) and 08-17 (43.3 kWh) are the two HIGHEST days in the 35-day record**, against a typical 25–35. **The excess matches the pump.**
  - **🎯 SO THE METHOD IS MORE TRUSTWORTHY THAN THIS ENTRY FIRST CONCLUDED, NOT LESS.** **A continuous load moved the FLOOR; a cycling load (the aircon) moved the SPREAD — exactly as theory predicts, and the two are separable in the same signal.** ➡️ **`load_power` at 5-minute polling CAN identify individual large loads on this property.** **What it still cannot do is close item 5(a) on its own**, because the aircon modulates rather than switching cleanly — the off-test below remains the way to a number.
  - **⚠️ DOMAIN NOTE: the borehole is SHARED/PERSONAL infrastructure, not Fungi4u.** **Recorded here only because it is required to read this analysis correctly** *(the same basis as `SOLAR.md`)*. ➡️ **The recurring cost, and an obvious cheap guard — an HA alert when the load floor stays >~1.2 kW above baseline for 30+ min — belong in `personal/`, not here. Raise it in a personal session.**
...
- **⛔⛔ THE BATTERY LEVER I ESTIMATED AT R500–770/mo IS WITHDRAWN — MEASURED AND DEAD, SAME DAY. ✏️ 2026-08-18.** *(Original claim, kept so it is not re-derived: `SOLAR.md` records the inverter in "emergency-backup posture" rather than self-consumption, so an uncycled 5.12 kWh battery looked like ~5 kWh/day of undisplaced grid import.)* **The claim was written while believing nothing solar was in HA. It is — see below — and the live numbers kill it.**
  - **✅ THE POSTURE HALF IS CONFIRMED: the battery IS idle.** `battery_soc` **99%**, `battery_discharge_today` **0.1 kWh**, `battery_charge_today` **0.3 kWh** — on a 5.12 kWh pack *(2026-08-18 11:00)*.
  - **🔴 BUT THE LEVER NEEDS SURPLUS PV AND THERE IS NONE. `grid_export_today` = 0.0 kWh, UNCHANGED FOR 98 HOURS.** Today by 11:00: **PV 2.8 kWh against grid import 11.9 kWh**, load 706 W with PV at 800 W. ➡️ **Winter PV never exceeds load, so there is no surplus to store.** ⛔ **And there is NO time-of-use tariff — the R5.12/kWh is the top INCLINING BLOCK, which prices by monthly volume, not by hour.** ➡️ **So cycling the battery in winter would be a grid → battery → load round trip at ~90% efficiency: it LOSES money.** **The lever may exist in summer at higher PV; on this data it does not exist now, and nothing should be spent on it.**
  - **✅ WHAT SURVIVES, AND IT VALIDATES THE MARGINAL RATE ALREADY IN USE: an inclining block means every kWh cut is worth the TOP block rate.** **R5.12/kWh is the right number for a REDUCTION** — it is the *timing* argument that fails here, not the reduction one.
- **✅✅ AND THE PREMISE OF THE "BUY THE SUNSYNK BRIDGE" RECOMMENDATION WAS WRONG — IT IS ALREADY BUILT, DEPLOYED AND LIVE. DO NOT SPEND THE R750–1,100.** `SOLAR.md` records the custom **`sunsynk-openapi-addon`** as *"tested end-to-end… it just needs deploying"*; **it has since been deployed.** **Verified 2026-08-18: all 15 sensors present under `sensor.sunsynk_inverter_cloud_*` and updating on a ~5-minute poll** — PV, load, grid import/export, battery SOC/power/current/temperature, and the daily kWh totals. ➡️ **The Waveshare bridge would buy RESILIENCE and RESOLUTION, not information** *(no internet dependency, sub-minute polling, no custom code)*. **`SOLAR.md`'s own verdict stands: "coarse 5-min cloud data is fine for the actual purpose."**
  - **⛔ ✏️ WITHDRAWN SAME DAY — "THE RECORDER HOLDS SUNSYNK HISTORY ONLY TO 08-09" WAS WRONG, AND THE FAULT WAS IN THE QUERY, NOT THE SYSTEM.** *(Original claim, kept because the reasoning pattern is the one to avoid: a 10-day history query returned rows only for the first day, which was read as an outage coinciding with the 08-09 fibre failure — a tidy story that fitted.)* **🔑 CAUSE: HA's history endpoint defaults `end_time` to `start_time + 1 DAY` when none is given, so a query written for ten days silently returns one.** ➡️ **Day-by-day counts show Sunsynk recording normally throughout: 105–171 rows/day, every day.** ⚠️ **Always pass `end_time` to the history API.**
  - **✅ AND THE ACTUAL LIMIT IS A DIFFERENT, MORE USEFUL FACT: HISTORY PURGES AT ~10 DAYS, BUT LONG-TERM STATISTICS ARE KEPT FOREVER.** *(2026-08-07 returns 0 rows for every entity; 08-08 onward is full.)* **All 15 Sunsynk sensors carry a `state_class`, so HA is already retaining hourly and daily statistics indefinitely — 35 days exist, back to the add-on's deployment on 2026-07-15.** 🔴 **THIS IS LOAD-BEARING FOR PHASE 0: the Sept–Nov and Dec–Mar reads span MONTHS, and `room_check.py` / `outdoor_history.py` both use the history API, which cannot see past ~10 days.** ➡️ **New tool for it: `stock-control/tools/ha_stats.py` — statistics are websocket-only, there is no REST route.**
  - **📊 AND THE 35-DAY RECORD SETTLES THE BATTERY QUESTION PROPERLY, NOT OFF ONE SNAPSHOT.** Daily: **PV 8–13 kWh · grid import 25–43 kWh · grid export 0.0 · battery discharge 0.1 kWh · SOC pinned 99–100%.** ➡️ **PV never approaches load, so the no-surplus finding above holds over five weeks, not one morning.**
  - **✅ AND THE BATTERY IS HEALTHY — IT IS CONFIGURED THIS WAY, NOT BROKEN.** On **2026-08-11 it discharged 2.30 kWh and SOC fell to 55%** — it did its backup job on the one day it was needed, then recharged. **So this is a strategy question with a measured answer of "no gain in winter", not a fault to fix.**
  - **⚠️ ONE NUMBER FOR `CASHFLOW.md` TO CARRY: grid import runs ~27 kWh/day ≈ 822 kWh/month**, against the ~715 kWh/mo that file records. **Winter, and the figure is measured rather than apportioned.**
- **➡️ SO THE HONEST RANKING IS MEASUREMENT-FIRST, AND THE TWO INSTRUMENTS COST ABOUT THE SAME.**
  1. **⛔ ✏️ PRICE CORRECTED 2026-08-18 — THE SHELLY IS ~R1,600–1,700, NOT R720.** *(The R720 came from a search snippet that did not match the retailer's own page. Verified by opening them: **SmartPad R1,699** (1 in stock) · **FirstShop R1,699** (out of stock) · Audico ~R1,560–1,620 (unverified, rate-limited). Nology is the official SA distributor.)* ➡️ **At that price it is no longer the cheap obvious buy, and it is now MORE than the UNI-T UT202A+ handheld at R950** *(which still measures the wrong thing — this file's rule is "read kWh over 24h, NOT instantaneous watts")*.
  2. **🎯 SO TRY THE FREE ROUTE FIRST, AND IT USES DATA ALREADY ON THE PI: EXTRACT THE AIRCON FROM WHOLE-PROPERTY `load_power`.** The Sunsynk add-on publishes **`sensor.sunsynk_inverter_cloud_load_power` every ~5 minutes**. **The aircon is a large cycling load against a ~1.1 kW base**, so its duty cycle should be recoverable as step changes — **overnight especially, when the rest of the property is quiet.** *(Same shape as the CO2-decay fit that measured the fan's flow for R0 — a differential over one instrument, not an absolute.)* ⚠️ **5-minute polling may be too coarse to resolve short cycles, and the freezers cycle too.** **If it bounds the number well enough to decide, the R1,700 is saved; if it does not, buy the meter knowing why.**
  3. **⛔ BUT IT IS BLOCKED ON THE RECORDER GAP BELOW — there is currently no Sunsynk history to analyse.** **Fix that first; it is free and it is a prerequisite either way.**
  4. **⬜ Shelly EM Gen3 + 50 A CT, ~R1,600 — only if step 2 fails.** Still the right instrument if bought: logs kWh to HA, permanent, doubles as the Phase 0 summer-load instrument, and **takes 2 CTs so one unit can split the aircon from the room's multiplug feed.**
  5. **⛔ Sunsynk telemetry bridge, R750–1,100 — DO NOT BUY. Already built, deployed and live** *(see the correction below)*.
