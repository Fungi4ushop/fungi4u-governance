...
### 🎛️ AN ESP-DRIVEN OUTSIDE-AIR INTAKE TO THE PLENUM — the actuator this room has never had. Operator proposal, 2026-08-14

**Operator, 2026-08-14:** *"a mechanism that gets controled by the esp to allow outside air into the plenum seems to give the most benifit during winter."* **✅ Correct, and the seasonal table below supports it: sub-18 °C air is available 100% of winter hours from 18:00 to 09:00. It is also the missing half of the control problem — the warm side has an actuator and the cold side does not.**

**🔑 AND THE REAL GAIN IS NOT "CONTROL", IT IS *WHERE THE COLD AIR ARRIVES*.** Today outside air reaches the room two ways, and **neither delivers it usefully**: *(1)* indirectly through the **15 mm hole → grow room → fresh-air fan → plenum**, by which point it has been tempered to ~22 °C and **is no longer cold**; *(2)* raw through the **front-wall grate and 50 mm strip**, which dumps it **at floor level onto the bottom shelf** — the documented cause of the overnight cold/dry layer. ➡️ **A direct outside→plenum duct is the only path that delivers cold air MIXED AND RE-HUMIDIFIED before it touches a bag**, because the plenum is where mixing and humidification happen. **That is the design argument, and it is stronger than the control argument.**

**🎯🎯 ARCHITECTURE CORRECTED BY THE OPERATOR, 2026-08-14 — AND IT IS BETTER THAN WHAT THIS BLOCK FIRST PROPOSED. THE DAMPER MODULATES; THE FAN RUNS CONSTANT.** *"I thought the damper must be esp controled to regulate the mix of internal and external air. Once we know what the speed of the fresh air fan must be it probably can stay constant."* **➡️ This is a constant-volume fan with a modulating mixing damper, and it dissolves the problem this file has been circling since 07-24.**

- **🔑 IT SEPARATES TWO JOBS THAT HAVE ALWAYS SHARED ONE ACTUATOR.** `MICROCLIMATE.md` §142 states the trap exactly: *"The fresh-air fan is not a single-purpose CO2 device — it simultaneously governs CO2 venting, temperature coupling, grow-room ventilation, positive pressure, and the humidifier drying load. These pull against each other."* **Modulating temperature with FAN SPEED moves all five at once** — which is why every fan experiment here has been unreadable. ➡️ **A mixing damper changes the *temperature* of the supply without changing its *volume*. Total flow — and with it positive pressure, CO2 exchange, grow-room ventilation and moisture delivery — stays put while temperature is controlled independently. Two actuators for two independent jobs, which is the right number.** **✏️ The variable-speed-fan proposal written above is superseded: it would have re-coupled everything it was meant to fix.**
- **✅ AND IT RETIRES THE VARIAC ARGUMENT IN ITS FAVOUR RATHER THAN AGAINST IT.** `DECISIONS.md` 2026-07-29 already concluded the fan wants *"a **fixed** point… a variable-speed fan lets it be dialled in precisely and left there — open-loop, no sensor authority, all of the benefit and none of the risk."* **That is precisely the operator's "once we know what the speed must be it probably can stay constant."** ➡️ **The owned-but-unfitted variac is the right part for the fan half, at R0**, and the ESP drives the damper instead. **The two decisions were never in conflict; they were about different halves.**
- **💰 SO IT IS CHEAPER THAN THE VERSION ABOVE — THE INSTALLED FAN IS REUSED.** The build becomes **an outside-air duct + a mixing box teeing it into the existing grow-room intake + a damper + a servo**, feeding the **fan already installed**. **No new fan.** *(ESPHome drives a hobby servo natively; a butterfly damper on a small servo is the right scale for a one-person operation — do not specify a commercial HVAC damper actuator.)*
- **⛔ FAIL-SAFE IS DAMPER-TO-INTERNAL, AND GETTING THIS BACKWARDS IS THE ONE FAILURE THAT COULD LOSE A CROP.** **Stuck full-OUTSIDE in winter floods the room with ~6 °C air and crashes it cold and dry within hours.** **Stuck full-INTERNAL merely drifts warm — which is today's state, and survivable.** ➡️ **On sensor loss, power-up, or any implausible reading, drive to INTERNAL.** *(This is the concrete form of 07-29's "fail-to-known-good", which until now had no defined position.)*
- **⛔ AND NEVER 100% OUTSIDE, EVEN WHEN IT WOULD HELP — THE GROW ROOM'S ONLY VENTILATION RUNS THROUGH THIS PATH.** `MICROCLIMATE.md`: outside air reaches the grow room through a 15 mm hole and is drawn onward by this fan, so **the grow room is ventilated as a side effect of the fruiting room's draw.** For the five days that fan was out (07-18→07-23) the grow room went **stuffy**. ➡️ **The damper needs a MINIMUM INTERNAL FRACTION in firmware, alongside the minimum-flow floor. Two floors, two different failure modes.**
- **✅ CO2 ALIGNS HELPFULLY; HUMIDITY IS THE ONLY REAL COST.** Opening toward outside makes the supply **colder, fresher AND lower-CO2 at the same time** — the cold source *is* the fresh source, so the temperature and CO2 jobs pull the same way rather than against each other. **The one thing it spends is humidifier duty** *(~7.7 g of water per m³ admitted)*, which stays the binding trade-off.
- **⬜ ONE LIMITATION, AND THE FIX IS FREE.** A constant fan cannot give the **extra** flow that summer's 04:00–06:00 night-grab wants, since the speed will be set for CO2 and pressure. ➡️ **The fan's 2-speed switch — useless as a fine actuator, and the thing that voided the 08-13 test — is exactly right as a coarse night-flush boost: LOW normally, HIGH for the summer pre-dawn window.** **The part that has frustrated every experiment here finally has a job it fits.**

**🌡️ WHAT THE DAMPER ACTS ON — operator's question, 2026-08-14: *"The working of the butterfly valve will probably require a temp sensor to act on."* ✅ Yes, and it is THREE temperatures with three different jobs. Only one of them is the setpoint variable, and only one of them has to be bought.**

| role | sensor | status |
|---|---|---|
| **Controlled variable** — what the loop is *for* | fruiting-room primary RS485 `SEN0438`, top-shelf height | ✅ installed |
| **Feedforward, warm side** | grow-room DHT22 (GPIO33) | ✅ installed |
| **Feedforward, cold side** | **outdoor air** | ⛔ **DOES NOT EXIST** |
| **Verification** — the damper's "tacho" | mixed-air, in the mixing box | ⛔ does not exist |

- **⛔ THE OUTDOOR SENSOR IS THE ONE REAL GAP, AND IT IS OVERDUE FOR REASONS OLDER THAN THIS DESIGN.** `weather.forecast_home` is **verified wrong** — it tracks the **daily maximum**, read 20.9 °C against a measured 12.4 °C on 2026-08-01, and would have inverted the sign of the stack ΔT on a dashboard. `outdoor_conditions_sensors.yaml` says it outright: *"a real outdoor sensor is the only honest fix."* **The vent-experiment arms A–E were confounded because outdoor temperature was not being recorded**, and `outdoor_history.py` only works **retrospectively — which a controller cannot use.** ➡️ **This one part closes a measurement gap that has already cost this project several experiments, and it is a prerequisite for the damper rather than a nice-to-have.**
- **🔑 WHY FEEDFORWARD RATHER THAN A PLAIN ROOM THERMOSTAT — the damper's AUTHORITY changes by an order of magnitude across the year.** The mix is simple arithmetic: **`T_mix = f·T_out + (1−f)·T_grow`**, so **`f = (T_grow − T_target) / (T_grow − T_out)`**. **At 6 °C outside against a 22 °C grow room, 20% open is a large cooling effect. At 17 °C outside on a summer pre-dawn, 100% open barely moves anything.** ➡️ **A fixed-gain loop on room temperature alone is wildly over-gained in winter and under-gained in summer — it will hunt in July and do nothing in January.** **The formula fixes that with arithmetic instead of tuning, which is the cheaper kind of engineering.** Then a **slow trim on the room sensor** corrects the residual, because the room has hours of lag and must not be chased.
- **✅ AND THE MIXED-AIR SENSOR IS THE DAMPER'S TACHO — this project has been burned twice by exactly this failure shape.** `HARDWARE_REFERENCE.md`'s standing complaint is *"no tacho on either fan — HA cannot tell a spinning fan from a dead one"*, and a dead fan sat displaying **"82%"** for ~24 hours. **A cheap servo has no position feedback, so a commanded damper is the same blind actuator again.** ➡️ **If commanded position moves and mixed-air temperature does not, the damper is stuck — and that is alarmable.** **Place it in the mixing box, BEFORE the humidifier**, or evaporative cooling from the fog will corrupt the reading.
- **✅ PUT THEM ON THE RS485 BUS — operator's preference, 2026-08-14, and the firmware turns out to be READY FOR IT.** *(✏️ This supersedes the 1-Wire DS18B20 suggestion written above, which would have introduced a second sensor standard for no gain.)* **Checked in `fruiting-room-controller.yaml`: the hand-rolled Modbus is already GENERIC.** `query_device(addr, reg_count)` and `find_frame(raw, addr, byte_count, out)` are reusable lambdas carrying the CRC16, the self-echo flush and the frame scan — **and the two installed sensors are just two call sites.** ➡️ **Adding a device is two lines plus a sensor definition, and `HARDWARE_REFERENCE.md` records Modbus address 3 as free.** **The handover argument is the real one: one bus, one protocol, one documented addressing scheme is what a successor can pick up.**
  - **🎯 USE A SECOND `SEN0438` FOR THE MIXED-AIR POINT — it is the part already proven in this exact environment.** It sits in the room's 90–95% RH today and works, so the condensing-airstream question is already answered. **Its block is a copy-paste: `query_device(0x03, 0x02)` + `find_frame(…, 0x03, 0x04, …)`.** ➡️ **And it returns humidity as well as temperature, free** — which is worth having at the mixing point.
  - **🔑 SO THE OUTDOOR SENSOR SHOULD BE TEMP *AND* RH TOO, NOT TEMP-ONLY — IT TURNS THE DAMPER'S WATER COST FROM AN ASSUMPTION INTO A MEASUREMENT.** The whole humidity trade rests on outside air being ~6 g/m³, which is currently **assumed**. `outdoor_history.py` computes outdoor AH precisely because *"it tests an assumption the vent argument rests on."* ➡️ **Measure it and the guard can compute the real moisture cost per m³ admitted instead of carrying a constant.** ⚠️ **The bare module is not weatherproof — it needs a shielded, ventilated enclosure. Mount it on the SOUTH face: Pretoria is at −25.8°, the sun sits to the NORTH, so south-facing walls are the shaded ones.**
  - **⚠️ THREE CAVEATS, AND THE FIRST IS THE ONE THAT COULD BLIND THE ROOM.** *(1)* **`query_device` hardcodes the start register to `0x0000`** *(query bytes 2–3)*. A device whose data sits elsewhere needs a start-register parameter — **add it with a DEFAULT of 0 so the two existing call sites stay byte-for-byte unchanged.** **That helper is shared with the room's only temp/RH and CO2 sensors; a careless edit blinds the room.** *(2)* **Each device costs a blocking `delay(150)`** inside a `30s` interval — **300 ms today, 600 ms at four devices.** Already past ESPHome's 30 ms guidance and evidently fine, but it **scales linearly**; if it ever bites, stagger the polls across intervals rather than growing one cycle. *(3)* **Ride the reflash that is already queued** — item 4b(c)'s uptime sensor and the pending `0x006B` CO2 offset — **and physically confirm BOTH existing sensors still report before walking away.**
  - **💰 PRICED 2026-08-14 (operator): `SEN0438` = R653.20 at Micro Robotics.** **First cost figure this project has ever had for an RS485 sensor.** ➡️ **Two of them is R1,306.40**, plus an outdoor enclosure, plus the damper, servo, duct and mixing box. *(Also confirm the extra devices fit inside **Fuse 6's 0.5 A** on TB7's 12 V branch, and give new runs **new W-numbers** — never renumber installed wiring.)*
    - **⛔ IT DOES NOT PAY BACK ON ELECTRICITY, AND THE CASE SHOULD NOT BE MADE THAT WAY.** The only directly monetisable benefit is humidifier duty. At the measured **120 W** and the marginal **R5.12/kWh**: **100% duty = R442/mo**, and **72.4%** *(the 08-11 figure, the best this room has recorded)* **= R320/mo — a saving of ~R122/mo.** **Against the sensors alone that is ~11 months; against the whole build it is longer, and the build is not priced.** ➡️ **The case is CROP — the room is at 19 °C against a 15–18 band, and `MICROCLIMATE.md` calls temperature the highest control priority. Make it on that, or not at all.**
    - **⚠️ AND R1,306.40 IS THE ONLY PRICED LINE IN THIS BUILD.** The **damper, servo, duct, mixing box and outdoor enclosure have never been costed**, and the fan and variac are R0 only because they are already owned. **Do not let one known number anchor a decision on an unknown total** — that is the failure this file's own standing rule exists to prevent.
    - **🎯 SO SPLIT THE PURCHASE — BUY THE OUTDOOR ONE NOW, DEFER THE MIXED-AIR ONE TO THE BUILD.** **They are not the same kind of spend.** The **mixed-air probe is verification for a damper that does not exist yet** — it can wait, and nothing degrades meanwhile. **The outdoor sensor earns its keep immediately and independently of whether the damper is ever built:** it is the control variable that **voided the 08-13 fan test**, that **confounded vent arms A–E**, and that `outdoor_conditions_sensors.yaml` already calls *"the only honest fix"*. ➡️ **R653.20 makes every future overnight experiment readable. That is worth having even if the damper is never fitted.**
    - **⬜ BUT PRICE A PURPOSE-BUILT WEATHERPROOF RS485 TEMP/RH PROBE BEFORE BUYING A `SEN0438` FOR OUTDOORS.** The bare module needs a **fabricated radiation shield**, and an agricultural/industrial probe is **built for the job and may land under R653 + enclosure.** ⚠️ **The cost of switching parts is a different register map** — but `query_device` needs its start-register parameter added anyway, so that work is already on the list. **Keep the `SEN0438` for the MIXED-AIR point regardless: there its proven survival in 90–95% RH and its identical register map are worth paying for.**
- **⛔ AND IT MUST NOT BE A TEMPERATURE-ONLY LOOP — the damper spends water.** Opening toward outside costs **~7.7 g of moisture per m³**. **A humidity guard must be able to override temperature demand and close the damper.** ➡️ **Compute it ON-DEVICE from the RS485 sensor's own temp and RH — `SAFETY.md` puts control logic local to the device, and the existing `fruiting_room_vpd` / `absolute_humidity` are HOME ASSISTANT templates, so a guard depending on them fails whenever the Pi does.** *(Judge it on VPD/AH, not raw RH — raw RH is temperature-confounded, and this damper changes temperature by design.)*
- **⚠️ CONTROL ON THE TOP-SHELF PRIMARY; TREAT THE SHELF DELTA AS AN INTERLOCK, NOT AN INPUT — AND CHECK IT FOR STALENESS.** The Inkbird is **"secondary/verification only, never control"** by standing decision, reports at **1 °C resolution**, and **its zero does not survive handling.** ➡️ **A bottom-shelf interlock is still worth having** *(the 07-25 crash was a bottom-shelf event)*, **but it inherits the Inkbird's unreliability: on 2026-08-09 it froze and the shelf-delta template kept producing plausible numbers straight through, voiding two nights.** **So the interlock must fail safe on staleness — stale or unavailable Inkbird ⇒ damper to INTERNAL, never "last known delta looked fine."**

**✅ WHAT IS GENUINELY FREE — the enclosure was built for this and the path is still there.** `HARDWARE_REFERENCE.md`: **TB2 / MOSFET2 / GPIO27** is a **12 V PWM-capable output, wired, fused (Fuse 4), and idle** — wires **W36–W41** all in place, driving nothing since the SEAFLO came out on 07-18. **Nothing in the enclosure needs rewiring for a 12 V fan.** ➡️ **And this is the first fan install where the tacho blindness can be fixed at build time rather than retrofitted: GPIO21 is free, and `HARDWARE_REFERENCE.md` already names a 4-pin PWM fan with its tacho on GPIO21 as the fix that "makes this failure alarmable instead of invisible."** **Two fans have already worn out unnoticed in the fresh-air position. Do not fit a third blind one.**

**⛔ AND THE ONE THING THAT WILL BE MISREAD AGAINST THIS: "THE PARTS ARE ALREADY OWNED, NOTHING NEEDS BUYING" DOES NOT APPLY HERE.** `MICROCLIMATE.md` §146 and `DECISIONS.md` 2026-07-29 cancelled a variable-speed fan purchase at **R0**, on the grounds that *"the installed mains inline fan on a variac delivers exactly that."* **That argument is about the FRESH-AIR POSITION, which draws from the GROW ROOM.** ➡️ **The variac modulates WARM air. This proposal needs COLD air, through a duct that does not exist.** **Different source, different duct, different job — the R0 finding does not transfer, and neither does "do not re-specify a DC blower for this position", which was decided against a fan already installed in the other position.**

**✅ AND IT DOES NOT REVERSE THE 07-29 "NO SENSOR AUTHORITY" DECISION — IT SITS OUTSIDE IT, BECAUSE THE SENSOR IS DIFFERENT.** `DECISIONS.md` 2026-07-29 refused the **CO2** sensor actuator authority, on the explicit grounds that *"a sensor that is not trusted must not be given control of an actuator"* — both CO2 sensors under-read by ~350 ppm **and agree with each other**, so their drift check is blind by construction. **None of that is true of temperature.** The primary RS485 temp/RH sensor is cross-checked against the Inkbird *and* the grow-room DHT22, has never taken a zero shift, and its shelf deltas are read every session. ➡️ **Temperature can legitimately hold actuator authority where CO2 cannot. Say so when this is built, or someone will read it as the declined loop coming back.**

**➡️ CARRY THE 07-29 PRECONDITIONS ANYWAY — they were written for exactly this and they are cheap:** a **plausibility clamp** *(refuse to act on a reading outside a sane band)*, **fail-to-known-good** *(sensor lost → go to a safe fixed opening, not to zero and not to full)*, and a **slew limit**. **⛔ Plus one this room learned the hard way: a MINIMUM-FLOW FLOOR.** Dropping the fresh-air fan to **off** on 2026-07-25 **crashed the bottom shelf within hours** — positive pressure is load-bearing. **The control law must not be able to command a net airflow below that floor.** *(An intake ADDS pressure, so this design helps rather than threatens it — but the floor must still be in the firmware, not in the operator's memory.)*

**⚠️ THE COST IS PAID IN HUMIDIFIER DUTY, AND THERE IS CURRENTLY ZERO HEADROOM.** Outside air runs **~6 g/m³** against the room's **13.7**, so **every m³ admitted costs ~7.7 g of water the humidifier must replace** — and it is **pinned at 100%** *(see the humidifier block)*. **✅ But the operating point is not hypothetical, it is in this record: on 08-11 the room held 16.65 °C at 90.65% RH on 72.4% duty — 28 points of headroom.** ➡️ **Cooling partly self-funds, because a cooler room needs less absolute moisture: the 92% RH requirement falls 14.99 → 13.30 g/m³ between 19 and 17 °C.** ⚠️ **Hinges on:** *that the duty freed by cooling exceeds the duty spent on the extra air exchange.* **Unmeasured, and it is the number that decides whether this works.** **Size of the move if wrong: the room ends cool and below 90% RH, trading a temperature fault for a humidity one.**

**⛔ AND IT IS ONLY HALF A JOB WITHOUT THE EXHAUST — this is the part that gets skipped.** `MICROCLIMATE.md`'s 07-23 design is **three** things: a controlled tempered intake, **the front 50 mm strip kept as the DEFINED EXHAUST, and the drain grate SEALED.** ➡️ **Fit the intake and leave the grate open and you have not replaced the uncontrolled path — you have added a controlled one in parallel with it**, and the weather still moves the room ~0.9 °C. **The control only becomes real when the uncontrolled path is closed.** *(⚠️ And that seal is the irreversible half — see item 10 on why it must be a damper and not a permanent cut.)*

**⬜ WINTER YES; SUMMER IT IS A NIGHT-GRAB, NOT A SOLUTION.** It captures the **04:00–06:00 window, the only free cooling a summer day offers** — worth having, and it makes the part dual-season rather than seasonal. **But it cannot touch the 95-hour January run when nothing outside is below 18 °C.** **Do not let this be recorded as answering the summer question.**

**⬜ NO PRICE HAS EVER BEEN RECORDED FOR THIS PART.** Standing rule, learned three times: **measure the baseline before costing an improvement.** ➡️ **Get a quote for a continuous-duty 4-pin PWM 12 V fan with tacho before committing** — and **⛔ not another SEAFLO: a bilge blower rated 600–700 h intermittent is a ~1–3 month consumable in a 24/7 position, and two have already died here.**
