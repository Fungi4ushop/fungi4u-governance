...
- **⚠️ THREE GTIN STEPS REMAIN, and the second is the one that actually breaks the trial.**
  1. **Scan-test a real printed punnet.** Must decode to exactly `6001651378474`. Test a printed label under normal light — **a screen always scans, so testing on screen proves nothing.** Usual failures are print quality, too small a quiet zone (the blank margin each side), or over-scaling down. **This is now the highest-risk item because the print run has already happened**; a failure here is a reprint. A phone scanner app is proportionate — formal ISO print-grading is overkill for a 4-week trial.
